...
__thread const int program_birth_year = 2000;

static __thread FILE *vstats_file;

__thread const char *const forced_keyframes_const_names[] = {
    "n",
    "n_forced",
    "prev_forced_n",
    "prev_forced_t",
...
static __thread int64_t video_size = 0;
static __thread int64_t audio_size = 0;
static __thread int64_t subtitle_size = 0;
static __thread int64_t extra_size = 0;
static __thread int nb_frames_dup = 0;
static __thread int nb_frames_drop = 0;
static __thread int64_t decode_error_stat[2];

static __thread int current_time;
...
{
    OptionDef local_options[140];
    if(!init_ffmpeg_options(local_options, 140))
	return -1;
