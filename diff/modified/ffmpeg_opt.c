...
static __thread int override_ffserver  = 0;

static __thread int64_t recording_time = INT64_MAX;
...
int init_ffmpeg_options(OptionDef * opts, int opt_count)
{
#define OFFSET(x) offsetof(OptionsContext, x)
    const OptionDef local_opts[] = {
	/* main options */
#include "cmdutils_common_opts.h"
    ...
    int real_opt_count = (sizeof(local_opts) / sizeof(local_opts[0]));
    if(opt_count != real_opt_count)
    {
	printf("Invalid number of opts, new = %d\n", real_opt_count);
	return 0;
    }

    memcpy(opts, local_opts, sizeof(local_opts));
    return 1;
}
